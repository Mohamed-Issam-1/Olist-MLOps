...
def make_runtime_model():
    return SimpleNamespace(
        ...
        classification_threshold=(0.0822110764307922),
    )
...
def test_health_route():
    ...
    assert body["service"] == ("Olist Late Delivery Inference API")
...
def test_model_info_route(
    ...
    runtime_model = make_runtime_model()
    ...
    assert body["source"] == "mlflow-registry"

    assert body["registered_model_name"] == "olist-late-delivery"
    ...
    assert body["classification_threshold"] == 0.0822110764307922
...
def test_model_info_returns_503_when_registry_unavailable(
    ...
    def fail_load():
        raise MlflowModelLoadError("registry unavailable")
    ...
    assert response.json() == {"detail": "Production model is unavailable."}
...
def test_predict_single_order(
    ...
    prediction_mock = Mock(
        return_value=pd.DataFrame(
            {
                "order_id": [("b3b54427f53d13f6063ef7007bf7d371")],
                "late_probability": [0.02952043625959755],
                "predicted_is_late": [0],
            }
        )
    )
    ...
    call_args = prediction_mock.call_args

    raw_order = call_args.args[0]
    ...
    assert raw_order.shape == (
        1,
        31,
    )

    assert raw_order.iloc[0]["order_id"] == ("b3b54427f53d13f6063ef7007bf7d371")

    assert call_args.kwargs["runtime_model"] is runtime_model


def test_predict_rejects_missing_required_field():
    payload = make_valid_order_payload()

    del payload["payment_total"]
...
def test_predict_rejects_extra_field():
    ...
    payload["future_delivery_result"] = "leakage"
...
def test_predict_rejects_invalid_timestamp():
    ...
    payload["order_purchase_timestamp"] = "not-a-timestamp"
...
def test_openapi_contains_prediction_route():
    ...
    assert "OrderPredictionRequest" in schema["components"]["schemas"]
    ...
    assert "BatchPredictionRequest" in schema["components"]["schemas"]

    assert "BatchPredictionResponse" in schema["components"]["schemas"]
...
def test_predict_batch(
    ...
    first = make_valid_order_payload()

    second = make_valid_order_payload()
    ...
    assert body["predictions"][0]["predicted_is_late"] == 0

    assert body["predictions"][1]["predicted_is_late"] == 1

    prediction_mock.assert_called_once()

    raw_orders = prediction_mock.call_args.args[0]

    assert raw_orders.shape == (
        2,
        31,
    )

    assert prediction_mock.call_args.kwargs["runtime_model"] is runtime_model
...
def test_predict_batch_rejects_oversized_batch(
    ...
    assert response.json() == {
        "detail": ("Batch size exceeds configured maximum of 1 orders.")
    }
...
def test_predict_batch_returns_503_when_registry_unavailable(
    ...
    response = client.post(
        "/predict/batch",
        json={"orders": [make_valid_order_payload()]},
    )
