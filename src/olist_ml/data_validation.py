...
def load_data_quality_config() -> dict[str, Any]:
    ...
    if not isinstance(
        ...
        raise DataValidationError(
            "Missing labeled_dataset data-quality configuration."
        )
    ...
    if missing:
        raise DataValidationError(
            "Data-quality configuration is missing: "
            + ", ".join(
                sorted(
                    missing
                )
...
def validate_target_consistency(
    ...
    if missing:
        raise DataValidationError(
            "Target consistency check is missing columns: "
            + ", ".join(
                sorted(
                    missing
                )
...
def validate_labeled_dataframe(
    ...
    *,
    ...
    if not isinstance(
        ...
        raise DataValidationError(
            "Labeled data must be a pandas DataFrame."
        )

    if dataframe.empty:
        raise DataValidationError(
            "Labeled data must not be empty."
        )
...
def validate_labeled_dataset(
    *,
    ...
    if not Path(
        ...
        raise DataValidationError(
            f"Labeled dataset does not exist: {dataset_path}"
        )
