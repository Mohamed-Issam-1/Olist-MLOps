    ...
    try:
        if suffix == ".csv":
            ...
            if (
                ...
                raise CliError(
                    "JSON input must contain one object or a non-empty list of objects."
                )

            orders = pd.DataFrame(payload)

        else:
            raise CliError("Unsupported input format. Use .json or .csv.")

    except CliError:
        raise

    except Exception as exc:
        raise CliError(f"Could not read input file: {input_path}") from exc

    if orders.empty:
        raise CliError("Input file contains no orders.")
...
def predict_file(
    ...
    runtime_model = load_registered_inference_model()
    ...
    result["model_version"] = runtime_model.version
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="olist-predict",
        description=(
            "Predict late Olist deliveries using the registered production model."
        ),
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help=("Path to a .json or .csv file containing raw order data."),
    )
...
def main(
    ...
    try:
        ...
        payload = serialize_predictions(predictions)
        ...
        if args.output is None:
            ...
            LOGGER.info(
                "cli_prediction_output_written path=%s rows=%d",
                args.output,
                len(predictions),
            )
